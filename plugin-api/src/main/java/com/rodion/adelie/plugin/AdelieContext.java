...
public interface AdelieContext {
  /**
   * Add service.
   *
   ...
   */
  <T extends AdelieService> void addService(final Class<T> serviceType, final T service);
   ...
   *       AdeliePlugin#start()} method is called
   ...
   *     is unavailable
   */
  <T extends AdelieService> Optional<T> getService(Class<T> serviceType);
}
