...
import com.rodion.pfm.plugin.services.PfmConfiguration;
...
public class DatabaseMetadata {
   ...
   *
   * @param pfmConfiguration portfolio manager configuration
   * @return the metadata to use for new db
   */
  public static DatabaseMetadata defaultForNewDb(final PfmConfiguration pfmConfiguration) {
    return new DatabaseMetadata(
        BaseVersionedStorageFormat.defaultForNewDB(pfmConfiguration.getDataStorageConfiguration()));
  }
  ...
  public static DatabaseMetadata lookUpFrom(final Path dataDir) throws IOException {
    logger.info("Lookup database metadata file in data directory: {}", dataDir.toString());
