...
public interface AdeliePlugin {
   ...
   * only time it will be provided to the plugin and is how the plugin will interact with Adelie.
   *
   * <p>Typically the plugin will not begin operation until the {@link #start()} method is called.
   *
   * @param context the context that provides access to Adelie services.
   */
  void register(AdelieContext context);
   ...
   * listener with Adelie services and starting any background threads the plugin requires.
   */
  void start();
