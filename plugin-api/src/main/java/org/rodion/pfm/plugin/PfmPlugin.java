...
public interface PfmPlugin {
    ...
    /**
     * Called when the plugin is first registered with Portfolio Manager. Plugins are registered very early in the
     * Portfolio Manager life-cycle and should use this callback to register any command line options required via
    ...
    /**
     * Called once when portfolio manager has loaded configuration but before external services have been started
     * e.g. metrics and http
     */
    default void beforeExternalServices() {}

    /**
     * Called once Portfolio Manager has loaded configuration and has started external services but before the main
     * loop is up. The plugin should begin operation, including registering any event listener with
     * Pfm services and starting any background threads the plugin requires.
     */
    void start();
    ...
    /**
     * Called when the plugin is being stopped. This method will be called as part of Portfolio Manager shutting
